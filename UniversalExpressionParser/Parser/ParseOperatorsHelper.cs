...
namespace UniversalExpressionParser.Parser
{
    internal class ParseOperatorsHelper
    {
            ...
            out List<IKeywordExpressionItem> parsedKeywords)
        {
            ...
            // Sort expressionLanguageProvider.Operators once and pass as parameter
            //var currentCandidateOperators = expressionLanguageProvider.Operators
            //                                                               //.Where(x =>
            //                                                               // ReSharper disable once AccessToModifiedClosure
            //                                                               //((int)x.OperatorType & (int)operatorTypeFlags) > 0)
            //                                                               .OrderByDescending(x => x.NameParts.Count).ToList();
            ...
            while (TryParseComments())
            {
                ...
                if (hasOperand1)
                {
                    //OperatorTypeFlags operatorTypeFlagsNew = OperatorTypeFlags.None;
                    ...
                    //else if ((lastMatchedOperatorFlags & OperatorTypeFlags.BinaryOperator) > 0)
                    //{
                    //    // We didn't find any postfix.
                    //    operatorTypeFlagsNew = OperatorTypeFlags.PrefixUnaryOperator;
                    //}
                    else
                    {
        ...
        // TODO: DELETE THE COMMENTED CODE AFTER THE FIRST COMMIT
        //private List<List<IOperatorInfoExpressionItem>> TryParseOperators_OLD([NotNull] ParseExpressionItemContext context, bool hasOperand1)
        //{
        //    OperatorTypeFlags operatorTypeFlags;
        //    OperatorTypeFlags lastMatchedOperatorFlags = OperatorTypeFlags.None;
        ...
        //    var currentCandidateOperators = expressionLanguageProvider.Operators.Where(x =>
        //                                                                   // ReSharper disable once AccessToModifiedClosure
        //                                                                   ((int)x.OperatorType & (int)operatorTypeFlags) > 0).OrderByDescending(x => x.NameParts.Count).ToList();
        ...
        //        Helpers.TryParseComments(context);
        ...
        //        // Try to find a list of best matched operators
        //        foreach (var operatorInfo in currentCandidateOperators)
        //        {
        //            if (lastMatchedOperatorInfo != null)
        //            {
        //                if (operatorInfo.NameParts.Count < lastMatchedOperatorInfo.NameParts.Count)
        ...
        //                if (lastMatchedOperatorInfo.NameParts.Count > 0)
        //                {
        //                    var ignoreOperator = false;
        ...
        //            if (lastMatchedOperatorInfo == null || operatorInfo.NameParts.Count > lastMatchedOperatorInfo.NameParts.Count)
        //            {
        //                var namePartsIndex = 0;

        //                if (lastOperatorInfoExpressionItem != null && lastMatchedOperatorInfo.NameParts.Count > 1)
        //                {
        //                    namePartsIndex = lastMatchedOperatorInfo.NameParts.Count - 1;
        //                    textSymbolsParser.MoveToToPosition(lastOperatorInfoExpressionItem.OperatorNameParts[namePartsIndex].IndexInText);
        //                }
        ...
        //                List<INameExpressionItem> operatorNamePartExpressionItems = null;
        ...
        //                    IsValidTextAfterMatchedTextDelegate isValidTextAfterTextToMatch;
        //                    if (namePartsIndex == operatorInfo.NameParts.Count - 1)
        //                        isValidTextAfterTextToMatch = Helpers.IsValidTextAfterTextToMatch;
        //                    else
        //                        isValidTextAfterTextToMatch = IsValidTextAfterMatchedTextMiddleOperatorName;

        //                    if (!Helpers.StartsWith(textSymbolsParser.TextToParse, namePart,
        //                        textSymbolsParser.PositionInText, textSymbolsParser.ParsedTextEnd,
        //                        expressionLanguageProvider,
        //                        isValidTextAfterTextToMatch, out var matchedText))
        //                        break;

        //                    var operatorNamePartExpressionItem = new SimpleExpressionItem(matchedText, textSymbolsParser.PositionInText, ExpressionItemType.Name);

        //                    if (operatorNamePartExpressionItems == null)
        //                        operatorNamePartExpressionItems = new List<INameExpressionItem>(operatorInfo.NameParts.Count);
        ...
        //                var currentOperatorInfoExpressionItem = new OperatorInfoExpressionItem(operatorInfo, operatorNamePartExpressionItems);
        //                if (IgnoreOperatorInfoDelegate != null && this.IgnoreOperatorInfoDelegate(currentOperatorInfoExpressionItem))
        //                    continue;
        ...
        //                lastOperatorInfoExpressionItem = currentOperatorInfoExpressionItem;
        //            }
        //            else if (operatorInfo.NameParts.Count == lastMatchedOperatorInfo.NameParts.Count)
        //            {
        //                var lastMatchedOperatorInfoLastNamePart = lastMatchedOperatorInfo.NameParts[^1];
        //                var currentOperatorInfoLastNamePart = operatorInfo.NameParts[^1];

        //                if (lastMatchedOperatorInfoLastNamePart.Length > currentOperatorInfoLastNamePart.Length)
        //                    continue;

        //                var lastPartPositionInText = lastOperatorInfoExpressionItem.OperatorNameParts[^1].IndexInText;

        //                if (!Helpers.StartsWith(textSymbolsParser.TextToParse, currentOperatorInfoLastNamePart,
        //                    lastPartPositionInText, textSymbolsParser.ParsedTextEnd,
        //                    expressionLanguageProvider,
        //                    null, out var matchedText))
        //                    continue;

        //                var operatorNamePartExpressionItems = new List<INameExpressionItem>(operatorInfo.NameParts.Count);

        //                for (var i = 0; i < lastOperatorInfoExpressionItem.OperatorNameParts.Count - 1; ++i)
        //                    operatorNamePartExpressionItems.Add(lastOperatorInfoExpressionItem.OperatorNameParts[i]);

        //                operatorNamePartExpressionItems.Add(
        //                    new SimpleExpressionItem(matchedText, lastPartPositionInText, ExpressionItemType.Name));
        ...
        //        var lastOperatorNamePartExpressionItem = lastOperatorInfoExpressionItem.OperatorNameParts[^1];
        ...
        //                operatorTypeFlagsNew |= (OperatorTypeFlags.BinaryOperator |
        //                                         OperatorTypeFlags.PostfixUnaryOperator);
        //            }
        //            //else if ((lastMatchedOperatorFlags & OperatorTypeFlags.BinaryOperator) > 0)
        //            //{
        ...
        //            if (operatorTypeFlagsNew != operatorTypeFlags)
        //            {
        //                operatorTypeFlags = operatorTypeFlagsNew;

        //                currentCandidateOperators = expressionLanguageProvider.Operators.Where(x =>
        //                    // ReSharper disable once AccessToModifiedClosure
        //                    ((int)x.OperatorType & (int)operatorTypeFlags) > 0).OrderByDescending(x => x.NameParts.Count).ToList();
        //            }
        //        }
